...
int main(int argc, char* argv[]) {
  ...
  if (strcmp(argv[i++], "est")==0) {
    ...
    cout << "Run BTM, K=" << K << ", A=" << A << ", W=" << W << ", alpha=" << alpha << ", beta=" << beta << ", n_iter=" << n_iter << ", save_step=" << save_step << " ====" << endl;	
    // load training data from file
	clock_t start = clock();
	Model model(K, A, W, alpha, beta, n_iter, save_step);
    ...
    int A = atoi(argv[4]);                  // author num
    string docs_pt(argv[5]);
    string dir(argv[6]);
    cout << "Run infence:K=" << K << " , A=" << A << ", type " << type << " ====" << endl;
    Infer inf(type, K);
    inf.run(docs_pt, dir);
  } else {
